...
*        and new PHI vintage / old MAD vintage for forecasting
*     2. Forecast using previous specification estimated on old data 
*        and new PHI vintage / new MAD vintage for forecasting
...
local estimates1_start 1999
local estimates1_stop 2023
local estimates2_start 1999
...
local forecast1_start 2025
local forecast1_stop 2035
local forecast2_start 2026
local forecast2_stop 2035
local forecast3_start 2026
local forecast3_stop 2036

* Define the estimation command for each model
local estimates1a_command = "regress lndiff_prem_real_demo L.lndiff_prem_real_demo lndiff_pdi_real_6yr if year >= `estimates1_start' & year <= `estimates1_stop'"
local estimates1b_command = "regress lndiff_prem_real L.lndiff_prem_real lndiff_pdi_real_6yr if year >= `estimates1_start' & year <= `estimates1_stop'"
local estimates2_command  = "regress lndiff_prem_real_demo L.lndiff_prem_real_demo lndiff_pdi_real_6yr if year >= `estimates2_start' & year <= `estimates2_stop'"
...
* Define adjustments
local adjustment_2024_prev = 0.02
local adjustment_2025_prev = 0.02
local adjustment_2024 = 0.02
local adjustment_2025 = 0.015
local adjustment_2026 = 0.015
...
di 			 "Parameters for pgm.do" 
	...
	char(10) "~~~ Adjustments ~~~"
	char(10) "Previous adjustment for 2024: `adjustment_2024_prev'"
	char(10) "Previous adjustment for 2025: `adjustment_2025_prev'"
	char(10) "Adjustment for 2024: `adjustment_2024'"
	char(10) "Adjustment for 2025: `adjustment_2025'"
	char(10) "Adjustment for 2026: `adjustment_2026'"
;
#delimit cr
...
gen pdi_real_`mad_vintage_prev' = inc_smooth_taxadj_cy_`mad_vintage_prev' / pop_cy_`mad_vintage_prev' / pc_cy_`mad_vintage_prev'			
gen lndiff_pdi_real_6yr_`mad_vintage_prev' = (ln(pdi_real_`mad_vintage_prev') - ln(L6.pdi_real_`mad_vintage_prev')) / 6

* Real premium growth (deflated by PCMED)
gen lndiff_pcmed_`mad_vintage_prev' = ln(pcmed_cy_`mad_vintage_prev') - ln(L.pcmed_cy_`mad_vintage_prev')
gen lndiff_prem_demo_`phi_vintage_prev' = ln(prem_demo_current_`phi_vintage_prev') - ln(L.prem_demo_current_`phi_vintage_prev')
gen lndiff_prem_real_demo_`phi_vintage_prev' = lndiff_prem_demo_`phi_vintage_prev' - lndiff_pcmed_`mad_vintage_prev'
gen lndiff_prem_`phi_vintage_prev' = ln(prem_current_`phi_vintage_prev') - ln(L.prem_current_`phi_vintage_prev')
gen lndiff_prem_real_`phi_vintage_prev' = lndiff_prem_`phi_vintage_prev' - lndiff_pcmed_`mad_vintage_prev'
...
gen lndiff_prem_demo_`phi_vintage' = ln(prem_demo_current_`phi_vintage') - ln(L.prem_demo_current_`phi_vintage')
gen lndiff_prem_real_demo_`phi_vintage' = lndiff_prem_demo_`phi_vintage' - lndiff_pcmed_`mad_vintage'
gen lndiff_prem_`phi_vintage' = ln(prem_current_`phi_vintage') - ln(L.prem_current_`phi_vintage')
gen lndiff_prem_real_`phi_vintage' = lndiff_prem_`phi_vintage' - lndiff_pcmed_`mad_vintage'

/* Estimate 1a: Previous baseline specification / Previous data / Demographic adjustment */
preserve
gen lndiff_prem_real_demo = lndiff_prem_real_demo_`phi_vintage_prev'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage_prev'
...
preserve
gen lndiff_prem_real = lndiff_prem_real_`phi_vintage_prev'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage_prev'
...
preserve
gen lndiff_prem_real_demo = lndiff_prem_real_demo_`phi_vintage'
...
preserve
gen lndiff_prem_real = lndiff_prem_real_`phi_vintage'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage'
...
/* ~~~ Step 0 (Previous baseline): Forecast using previous specification estimated on old data and old PHI vintage / old MAD vintage for forecasting ~~~ */
...
gen lndiff_prem_real_demo = lndiff_prem_real_demo_`phi_vintage_prev'
gen prem_demo = prem_demo_`phi_vintage_prev'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage_prev'
gen lndiff_pcmed = lndiff_pcmed_`mad_vintage_prev'

/* Boost the 2024 and 2025 values in response to feedback from stakeholder interviews */
replace lndiff_prem_real_demo = lndiff_prem_real_demo + `adjustment_2024_prev' if year == 2024
...
foreach y of numlist `forecast1_start'/`forecast1_stop' {
	...
	if `y' == 2025 {
		replace lndiff_prem_real_demo = lndiff_prem_real_demo + `adjustment_2025_prev' if year == 2025
	}
}
...
* Generate variables for the Spring 2025 baseline's specification
gen lndiff_prem_real = lndiff_prem_real_`phi_vintage_prev'
gen prem = prem_`phi_vintage_prev'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage_prev'
gen lndiff_pcmed = lndiff_pcmed_`mad_vintage_prev'

/* Boost the 2024 and 2025 values to account for the effect of GLP1s on private spending */
replace lndiff_prem_real = lndiff_prem_real + `adjustment_2024_prev' if year == 2024
...
foreach y of numlist `forecast1_start'/`forecast1_stop' {
	...
	if `y' == 2025 {
		replace lndiff_prem_real = lndiff_prem_real + `adjustment_2025_prev' if year == 2025
	}
}
* Convert to the percentage change growth rate of nominal premium growth
...
/* ~~~ Step 1: Forecast using previous specification estimated on old data and new PHI vintage / old MAD vintage for forecasting ~~~ */
preserve 
estimates restore estimates1a

* Generate variables for the Spring 2025 baseline's specification
gen lndiff_prem_real_demo = lndiff_prem_demo_`phi_vintage' - lndiff_pcmed_`mad_vintage_prev'
gen prem_demo = prem_demo_`phi_vintage'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage_prev'
gen lndiff_pcmed = lndiff_pcmed_`mad_vintage_prev'
...
/* ~~~ Step 2: Forecast using previous specification estimated on old data and new PHI vintage / new MAD vintage for forecasting ~~~ */
preserve 
estimates restore estimates1a
...
/* ~~~ Step 3: Forecast using previous specification estimated on new data and new PHI vintage / new MAD vintage for forecasting ~~~ */
preserve 
...
/* ~~~ Step 4: Forecast using new specification estimated on new data and new PHI vintage / new MAD vintage for forecasting ~~~ */
...
* Generate variables for the Winter 2026 baseline's specification
gen lndiff_prem_real_demo = lndiff_prem_real_demo_`phi_vintage'
gen prem_demo = prem_demo_`phi_vintage'
gen lndiff_pdi_real_6yr = lndiff_pdi_real_6yr_`mad_vintage'
gen lndiff_pcmed = lndiff_pcmed_`mad_vintage'
...
foreach y of numlist `forecast3_start'/`forecast3_stop' {
	...
	replace lndiff_prem_real_demo = prediction if year == `y'
	if `y' == 2026 {
		replace lndiff_prem_real_demo = lndiff_prem_real_demo + `adjustment_2026' if year == 2026
	}
}
* Convert to the percentage change growth rate of nominal premium growth
gen pchange_prem_demo = exp(lndiff_prem_real_demo + lndiff_pcmed) - 1
...
foreach y of numlist `forecast3_start'/`forecast3_stop' {
	...
	replace lndiff_prem_real = prediction if year == `y'
	if `y' == 2026 {
		replace lndiff_prem_real = lndiff_prem_real + `adjustment_2026' if year == 2026
	}
}
* Convert to the percentage change growth rate of nominal premium growth
gen pchange_prem = exp(lndiff_prem_real + lndiff_pcmed) - 1
...
keep year pchange_prem_demo_current_`phi_vintage_prev' pchange_prem_demo_current_`phi_vintage' ///
